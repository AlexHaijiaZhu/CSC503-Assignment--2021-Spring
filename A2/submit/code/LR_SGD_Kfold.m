...
Cs = [0.1,1,10,50,100,1000];
t0s = [1e-3, 1e-2, 1e-1,1, 5,1e1,1e2,5e2,1e4, 5e4];
alphas = [1e-1,1, 5,1e1,1e2,5e2,1e4, 5e4];
no_epoch = 5;
% init
mean_accuracies = zeros(length(alphas),length(t0s));
accuracies_in_test = zeros(length(alphas),length(t0s)); % for each gamma
test_accuracy = zeros(length(alphas),length(t0s));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% main
...
for i = 1:length(alphas)
    i
    alpha = alphas(i);
    for j = 1:length(t0s) % find optimal t0 using K fold
        t0 = t0s(j);
        % and perform k-fold
...
%             loss_in_cell{j,n} = loss;
        ...
        mean_accuracies(i,j) = mean(accuracies_in_k);
            % retrain 
        loss = [];
        learn_rate = [];
        w=rand(size(XTrain(1,:)));
